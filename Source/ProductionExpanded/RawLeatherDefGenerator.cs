...
namespace ProductionExpanded
{
  ...
  public static class RawLeatherDefGenerator
  {
    ...
    public static Dictionary<ThingDef, ThingDef> FinishedToRawMap = new Dictionary<ThingDef, ThingDef>();
    public static Dictionary<ThingDef, ThingDef> RawToFinishedMap = new Dictionary<ThingDef, ThingDef>();
    ...
    private static void GenerateRawLeatherDefs()
    {
      ...
      int generated = 0;
      var allLeathers = DefDatabase<ThingDef>.AllDefs
        .Where(def => def.stuffProps != null &&
      ...
      Log.Message($"[Production Expanded] Generated {generated} raw leather definitions from {allLeathers.Count} finished leathers.");
    }
    ...
    private static ThingDef CreateRawLeatherDef(ThingDef finishedLeather)
    {
      ...
      if (Prefs.DevMode)
      {
        Log.Message($"[Production Expanded] Creating raw leather for {finishedLeather.defName}: category={category}, size={size}, texPath={texturePath}");
      }
      ...
      var rawLeather = new ThingDef
      {
        ...
        statBases = new List<StatModifier>
        {
          ...
          new StatModifier { stat = StatDefOf.MarketValue, value = finishedLeather.BaseMarketValue * 0.4f }  // 40% value of finished
        },
        ...
        tradeTags = new List<string> { "PE_RawLeather" },
